...
Shader "Custom/SkyboxCubemap" {
	Properties{
		_SpaceColor("Space Color", Color) = (.5, .5, .5, .5)
		[Gamma] _SpaceExposure("Space Exposure", Range(0, 8)) = 1.0
		...
		SubShader{
			...
			Pass {
				...
				//	half3 c = DecodeHDR(tex, _Tex_HDR);
					...
					half3 c = (( texNebulae * _SpaceColor.rgb * _SpaceExposure) + (texStars * _StarExposure));
					return half4(c, 1);
				}
				ENDCG
			}
